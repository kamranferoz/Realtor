...
if ( ! function_exists( 'generate_dynamic_css' ) ) {
	function generate_dynamic_css() {
		...
		if ( ! empty( $inspiry_features_background_image ) ) {
            ...
            <style type='text/css' id="dynamic-css-bg">
                .home-features-section .home-features-bg {
                    background-image: url(<?php echo esc_url( $inspiry_features_background_image_url ); ?>);
                }
            </style>
			<?php
		}
		...
		if ( $dynamic_font_css_count > 0 ) {
			echo "<style type='text/css' id='dynamic-css-fonts-classic'>\n\n";
			foreach ( $dynamic_font_css as $font_unit ) {
				if ( ! empty( $font_unit['value'] ) ) {
					echo strip_tags( $font_unit['elements'] . " { " . $font_unit['property'] . " : " . $font_unit['value'] . ";" . " }\n" );
				}
			}
			echo '</style>';
		}
