...
// Theme version.
define( 'INSPIRY_THEME_VERSION', '3.9.6' );
...
if(!function_exists('inspiry_country_flags_code_styles')){
	function inspiry_country_flags_code_styles(){
		wp_enqueue_style(
			'inspiry-country-flags-css',
			get_theme_file_uri( 'common/css/intlTelInput.min.css' ),
			array(),
			INSPIRY_THEME_VERSION
		);
	}
	add_action( 'wp_enqueue_scripts', 'inspiry_country_flags_code_styles' );
}

if(!function_exists('inspiry_country_flags_code_scripts')){

	function inspiry_country_flags_code_scripts(){
	wp_enqueue_script(
		'inspiry-country-flags-js',
		get_theme_file_uri( 'common/js/intlTelInput.min.js' ),
		array( 'jquery' ),
		INSPIRY_THEME_VERSION,
		false
	);
	}
	add_action( 'wp_enqueue_scripts', 'inspiry_country_flags_code_scripts' );
}